...
import type { LightgbmRolloutReranker } from "../../apps/server/src/providers/lightgbm-provider";
import type {
  RuntimeActionResult,
  RuntimeAdminService,
  RuntimeAdminStatus,
...
async function withServer<T>(
  ...
  const server = createAppServer({
    ...
    lightgbmRolloutReranker: options.lightgbmRolloutReranker,
...
describe("backend foundation server routes", () => {
  ...
  it("slims simulation-mode LightGBM response metadata by default while keeping telemetry detail", async () => {
    const scorer: LightgbmScorer = {
      async score(request) {
        return {
          scores: request.legalActions.map((_, index) => (index === 0 ? 0.9 : 0.1)),
    ...
    await withServer(
      async ({ baseUrl, repository }) => {
        ...
        const payload = (await response.json()) as {
          ...
          provider_used: string;
          metadata?: {
            simulation_response_detail?: string;
            scores?: Array<{ score: number }>;
            model_metadata?: unknown;
            runtime_metadata?: unknown;
            explanation?: unknown;
            state_features?: unknown;
            candidate_features?: unknown;
            top_k_candidate_scores?: Array<{ score: number }>;
          };
        };
        expect(payload.accepted).toBe(true);
        expect(payload.provider_used).toBe("lightgbm_model");
        expect(payload.metadata?.simulation_response_detail).toBe("slim");
        expect(payload.metadata?.scores).toBeUndefined();
        expect(payload.metadata?.model_metadata).toBeUndefined();
        expect(payload.metadata?.runtime_metadata).toBeUndefined();
        expect(payload.metadata?.explanation).toBeUndefined();
        expect(payload.metadata?.state_features).toBeUndefined();
        expect(payload.metadata?.candidate_features).toBeUndefined();
        expect(payload.metadata?.top_k_candidate_scores?.[0]?.score).toBe(0.9);
        await flushServerQueue();
        expect(repository.decisions).toHaveLength(1);
        expect(repository.decisions[0]?.policy_source).toBe("lightgbm_model");
  ...
  it("prefilters large trick-play LightGBM requests before full feature building", async () => {
    ...
    await withServer(
      ...
      },
      {
        lightgbmScorer: scorer,
        serverConfig: {
          lightgbmConfidenceDelegationMaxPreDelegationMs: null,
  ...
  it("keeps low-confidence rollout-ranker decisions on LightGBM when the delegation latency budget is exceeded", async () => {
    let scoreCalls = 0;
    const scorer: LightgbmScorer = {
      async score(request) {
        scoreCalls += 1;
    ...
    await withServer(
      async ({ baseUrl, repository }) => {
        ...
        const payload = (await response.json()) as {
          ...
          provider_used: string;
          metadata?: {
            lightgbm_confidence_delegation_skipped_reason?: string;
            lightgbm_confidence_delegation_pre_ms?: number | null;
            lightgbm_confidence_delegation_pre_ms_threshold?: number | null;
          };
        };
        expect(payload.accepted).toBe(true);
        expect(payload.provider_used).toBe("lightgbm_model");
        expect(payload.metadata?.lightgbm_confidence_delegation_skipped_reason).toBe(
          "pre_delegation_latency_budget_exceeded"
        );
        expect(payload.metadata?.lightgbm_confidence_delegation_pre_ms).toBeGreaterThan(
          5
        );
        expect(payload.metadata?.lightgbm_confidence_delegation_pre_ms_threshold).toBe(
          5
        );
        expect(scoreCalls).toBe(1);
        await flushServerQueue();
        ...
        ).toBe("pre_delegation_latency_budget_exceeded");
      },
      {
        lightgbmScorer: scorer,
        serverConfig: {
  ...
  it("chooses the rollout-reranked winner instead of the top raw LightGBM score", async () => {
    const scorer: LightgbmScorer = {
      async score(request) {
        return {
          scores: request.legalActions.map((_, index) => (index === 0 ? 0.95 : 0.5)),
          modelMetadata: {
            model_type: "lightgbm_action_model",
            objective: "rollout_ranker",
            feature_profile: "runtime_raw",
    ...
    await withServer(
      ...
      },
      {
        ...
        serverConfig: {
          ...
          lightgbmRolloutRerankMaxActorHandSize: null,
        }
      }
    );
  });
  ...
  it("skips rollout reranking when the actor still holds a large hand", async () => {
    const scorer: LightgbmScorer = {
      async score(request) {
        return {
          scores: request.legalActions.map((_, index) => (index === 0 ? 0.95 : 0.92)),
          modelMetadata: {
            model_type: "lightgbm_action_model",
            objective: "rollout_ranker",
            feature_profile: "runtime_raw",
    ...
    const reranker: LightgbmRolloutReranker = async ({ ranked }) => {
      ...
      return {
        ...
        candidateResults: ranked.slice(0, 2).map((candidate, index) => ({
          ...
          rolloutSamples: 1,
          rolloutFailures: 0,
          rolloutMeanActorTeamDelta: index === 0 ? -25 : 50,
          rolloutFailureReason: null,
        })),
      };
    };
      ...
      async ({ baseUrl, repository }) => {
        ...
        const payload = (await response.json()) as {
          ...
          metadata?: {
            lightgbm_rollout_rerank_applied?: boolean;
            lightgbm_rollout_rerank_skipped_reason?: string;
            lightgbm_rollout_rerank_actor_hand_size?: number | null;
            lightgbm_rollout_rerank_actor_hand_size_threshold?: number | null;
          };
        };
        expect(payload.accepted).toBe(true);
        expect(payload.provider_used).toBe("server_heuristic");
        ...
        expect(
          repository.decisions[0]?.metadata.lightgbm_rollout_rerank_skipped_reason
        ).toBe("actor_hand_size_above_threshold");
      },
      {
        lightgbmScorer: scorer,
        lightgbmRolloutReranker: reranker,
