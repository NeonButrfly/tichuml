...
describe("normal viewport table layout", () => {
  it.each(SAFE_GEOMETRY_VIEWPORTS)(
    "exposes safe geometry metrics and keeps hands inside them at %sx%s",
    ({ width, height }) => {
      ...
      const safeMetrics = metrics as typeof metrics &
        Partial<{
      ...
      const minimumSouthClearance = Math.max(
        8,
        Math.round(metrics.cardHeight * 0.08)
      );

      expect(safeMetrics.safeViewportLeft).toBeTypeOf("number");
      expect(safeMetrics.safeViewportRight).toBeTypeOf("number");
      expect(safeMetrics.safeViewportTop).toBeTypeOf("number");
      expect(safeMetrics.safeViewportBottom).toBeTypeOf("number");
      expect(safeMetrics.safeActionRowTop).toBeTypeOf("number");
      expect(safeMetrics.safeActionRowBottom).toBeTypeOf("number");
      expect(safeMetrics.trickCardWidth).toBeTypeOf("number");
      expect(safeMetrics.trickCardHeight).toBeTypeOf("number");
      expect(safeMetrics.wishPickerWidth).toBeTypeOf("number");
      expect(safeMetrics.wishPickerHeight).toBeTypeOf("number");

      expect(safeMetrics.safeActionRowTop).toBe(actionRowRect.top);
      expect(safeMetrics.safeActionRowBottom).toBe(actionRowRect.bottom);
      expect(safeMetrics.safeViewportLeft).toBeGreaterThan(board.left);
      expect(safeMetrics.safeViewportRight).toBeLessThan(board.right);
      expect(safeMetrics.safeViewportTop).toBeGreaterThan(board.top);
      expect(safeMetrics.safeViewportBottom).toBeLessThan(board.bottom);
      expect(safeMetrics.trickCardWidth).toBeLessThan(metrics.cardWidth);
      expect(safeMetrics.trickCardHeight).toBeLessThan(metrics.cardHeight);
      expect(safeMetrics.trickCardHeight).toBeGreaterThan(
        safeMetrics.trickCardWidth!
      );
      expect(safeMetrics.wishPickerWidth).toBeLessThanOrEqual(
        metrics.centerColumnWidth
      );
      expect(safeMetrics.wishPickerWidth).toBeGreaterThan(metrics.cardWidth);
      expect(safeMetrics.wishPickerHeight).toBeLessThanOrEqual(
        metrics.actionBandHeight
      );
      expect(safeMetrics.wishPickerHeight).toBeGreaterThan(0);

      expect(
        safeMetrics.safeActionRowTop! - southAnchor.handBounds.bottom
      ).toBeGreaterThanOrEqual(minimumSouthClearance);

      expect(westAnchor.handBounds.left).toBeGreaterThanOrEqual(
        safeMetrics.safeViewportLeft!
      );
      expect(westAnchor.handBounds.right).toBeLessThanOrEqual(
        safeMetrics.safeViewportRight!
      );
      expect(westAnchor.handBounds.top).toBeGreaterThanOrEqual(
        safeMetrics.safeViewportTop!
      );
      expect(westAnchor.handBounds.bottom).toBeLessThanOrEqual(
        safeMetrics.safeViewportBottom!
      );

      expect(eastAnchor.handBounds.left).toBeGreaterThanOrEqual(
        safeMetrics.safeViewportLeft!
      );
      expect(eastAnchor.handBounds.right).toBeLessThanOrEqual(
        safeMetrics.safeViewportRight!
      );
      expect(eastAnchor.handBounds.top).toBeGreaterThanOrEqual(
        safeMetrics.safeViewportTop!
      );
      expect(eastAnchor.handBounds.bottom).toBeLessThanOrEqual(
        safeMetrics.safeViewportBottom!
      );
    }
  );

  it("honors in-bounds canonical side-hand y anchors before applying safe clamping", () => {
    const metrics = computeNormalViewportLayoutMetrics({
      viewportWidth: 1600,
      viewportHeight: 900,
      topCount: 8,
    ...
    const adjustedLayout = {
      ...DEFAULT_NORMAL_TABLE_LAYOUT,
      westHand: {
        ...DEFAULT_NORMAL_TABLE_LAYOUT.westHand,
        y: 0.42
      },
      eastHand: {
        ...DEFAULT_NORMAL_TABLE_LAYOUT.eastHand,
        y: 0.58
      }
    };
    const westExpectedY = resolveNormalBoardAnchorPoint(
      adjustedLayout.westHand,
      metrics
    ).y;
    const eastExpectedY = resolveNormalBoardAnchorPoint(
      adjustedLayout.eastHand,
      metrics
    ).y;
    const westAnchor = resolveNormalSeatAnchorGeometry({
      position: "left",
      normalTableLayout: adjustedLayout,
      layoutMetrics: metrics,
      handCardCount: 8
    });
    const eastAnchor = resolveNormalSeatAnchorGeometry({
    ...
    expect(westExpectedY).toBeGreaterThan(metrics.safeViewportTop);
    expect(westExpectedY).toBeLessThan(metrics.safeViewportBottom);
    expect(eastExpectedY).toBeGreaterThan(metrics.safeViewportTop);
    expect(eastExpectedY).toBeLessThan(metrics.safeViewportBottom);
    expect(westAnchor.hand.y).toBeCloseTo(westExpectedY, 5);
    expect(eastAnchor.hand.y).toBeCloseTo(eastExpectedY, 5);
  });
