...
usage() {
  cat <<'EOF'
Usage: scripts/runtime-action.sh <action>
...
Examples:
  scripts/runtime-action.sh start_backend

Destructive warning:
  clear_db drops and recreates the public schema. The backend admin API gates access to this script.
EOF
}
