...
usage() {
  cat <<'EOF'
Usage:
  scripts/verify-full-sim-backend.sh [options]
...
Examples:
  ./scripts/verify-full-sim-backend.sh --clear-database --games 100 --provider local
  ./scripts/verify-full-sim-backend.sh --clear-database --games 25 --provider server_heuristic --telemetry-mode full --full-state true --require-full-state-raw
  ./scripts/verify-full-sim-backend.sh --games 1000 --provider local --strict-telemetry false
EOF
}
...
if [[ "$health_status" != "200" && "$START_BACKEND_IF_DOWN" = "true" ]]; then
  ...
  if [[ -x "$REPO_ROOT/scripts/start-backend.sh" ]]; then
    "$REPO_ROOT/scripts/start-backend.sh" > "$ARTIFACT_DIR/start-backend-linux.log" 2>&1 || true
  elif [[ -f "$REPO_ROOT/scripts/backend-common.sh" ]]; then
    bash -lc "source '$REPO_ROOT/scripts/backend-common.sh'; load_repo_env; start_backend_background" > "$ARTIFACT_DIR/start-backend-linux.log" 2>&1 || true
  else
