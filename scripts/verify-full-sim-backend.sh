...
if [[ "$health_status" != "200" && "$START_BACKEND_IF_DOWN" = "true" ]]; then
  ...
  if [[ -x "$REPO_ROOT/scripts/start_backend_linux.sh" ]]; then
    "$REPO_ROOT/scripts/start_backend_linux.sh" > "$ARTIFACT_DIR/start-backend-linux.log" 2>&1 || true
  elif [[ -x "$REPO_ROOT/scripts/linux/start-backend.sh" ]]; then
    "$REPO_ROOT/scripts/linux/start-backend.sh" > "$ARTIFACT_DIR/start-backend-linux.log" 2>&1 || true
  elif [[ -f "$REPO_ROOT/scripts/backend-linux-common.sh" ]]; then
    bash -lc "source '$REPO_ROOT/scripts/backend-linux-common.sh'; load_repo_env; start_backend_background" > "$ARTIFACT_DIR/start-backend-linux.log" 2>&1 || true
  else
