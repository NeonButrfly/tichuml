...
# Usage:
#   cd /opt/tichuml
#   ./scripts/reset-tichuml-state.sh
#
# Optional:
#   ./scripts/reset-tichuml-state.sh --repo-root /opt/tichuml --backend-url http://127.0.0.1:4310
...
usage() {
  cat <<'USAGE'
Usage:
  reset-tichuml-state.sh [options]
...
if [[ "$CONFIRMED" != "true" ]]; then
  fail "Refusing to clear Tichu state without --yes or --clear-database."
fi
...
if health_ok; then
  ...
  elif [[ -f "$REPO_ROOT/scripts/start-backend.sh" ]]; then
    bash "$REPO_ROOT/scripts/start-backend.sh"
  else
...
if [[ "$SKIP_BACKEND_STOP" == "true" ]]; then
  ...
  if [[ -x "$REPO_ROOT/scripts/stop-backend.sh" ]]; then
    "$REPO_ROOT/scripts/stop-backend.sh" || true
  elif [[ -f "$REPO_ROOT/scripts/stop-backend.sh" ]]; then
    bash "$REPO_ROOT/scripts/stop-backend.sh" || true
  else
