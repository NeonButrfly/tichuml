...
main() {
  ...
  if [ "${BACKEND_INSTALL_REEXECED:-false}" != "true" ]; then
    ...
    BACKEND_INSTALL_REEXECED=true \
      ...
      exec bash "$repo_root/scripts/linux/install-backend.sh" "$@"
  fi

  export BACKEND_REPO_ROOT="$repo_root"
  log_step "Loading shared Linux backend helper"
  # shellcheck disable=SC1090
  . "$repo_root/scripts/linux/backend-common.sh"
...
Next commands:
- Start/update backend: $repo_root/scripts/linux/start-backend.sh
- Stop backend: $repo_root/scripts/linux/stop-backend.sh --backend-only
- Full stop: $repo_root/scripts/linux/stop-backend.sh --full
- Check backend status: $repo_root/scripts/linux/status-backend.sh
