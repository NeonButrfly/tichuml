...
POSTGRES_CONTAINER_NAME="${POSTGRES_CONTAINER_NAME:-tichu-postgres}"
...
docker_compose() {
  ...
  if ! compose_command="$(docker_compose_command)"; then
    log_fail "Docker is installed but neither 'docker compose' nor 'docker-compose' is available. Rerun scripts/install-backend.sh so it can install a distro package or manual Compose plugin."
    exit 1
  fi
...
verify_node_workspace_dependencies() {
  ...
  if [ ! -d "$BACKEND_REPO_ROOT/node_modules" ]; then
    log_fail "Node workspace dependencies are missing. Run npm install or rerun scripts/install-backend.sh."
    exit 1
  fi
...
verify_runtime_artifacts() {
  ...
  if [ -n "$missing" ]; then
    ...
    log_fail "Recovery: run npm run build or scripts/update-backend.sh before migrations/startup."
    exit 1
  fi
