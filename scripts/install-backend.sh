...
main() {
  ...
  if [ "${BACKEND_INSTALL_REEXECED:-false}" != "true" ]; then
    ...
    BACKEND_INSTALL_REEXECED=true \
      ...
      exec bash "$repo_root/scripts/install-backend.sh" "$@"
  fi
...
Next commands:
- Start/update backend: $repo_root/scripts/start-backend.sh
- Stop backend: $repo_root/scripts/stop-backend.sh --backend-only
- Full stop: $repo_root/scripts/stop-backend.sh --full
- Check backend status: $repo_root/scripts/status-backend.sh
