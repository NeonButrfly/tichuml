...
# Usage:
#   cd C:\tichu\tichuml
#   powershell -ExecutionPolicy Bypass -File .\scripts\reset-tichuml-state.ps1
#
# Optional:
#   powershell -ExecutionPolicy Bypass -File .\scripts\reset-tichuml-state.ps1 -RepoRoot C:\tichu\tichuml -BackendUrl http://127.0.0.1:4310
...
@"
Usage:
  scripts\reset-tichuml-state.ps1 [options]

Purpose:
  Starts Postgres/backend if needed, clears the Tichu database through the backend admin endpoint, then clears runtime/log artifacts.
...
Options:
  ...
  -AdminConfirm <token>
  -SkipBackendStop
  -Yes or -ClearDatabase      Required destructive confirmation.
  -Help, -?, --help, -h

Examples:
  powershell -ExecutionPolicy Bypass -File scripts\reset-tichuml-state.ps1 -ClearDatabase
...
if (-not $Yes -and -not $ClearDatabase) {
  Write-Error "Refusing to clear Tichu state without -Yes or -ClearDatabase."
  exit 2
}
...
if ([string]::IsNullOrWhiteSpace($RepoRoot)) {
    ...
    $Candidate2 = Resolve-Path -LiteralPath "." -ErrorAction SilentlyContinue
...
if ($BackendWasRunning) {
    ...
    $StartScriptCandidates = @(
        (Join-Path $RepoRoot "scripts\start-backend.ps1"),
        (Join-Path $RepoRoot "scripts\start-backend.ps1")
    )
...
if ($SkipBackendStop) {
    ...
    $StopScriptCandidates = @(
        (Join-Path $RepoRoot "scripts\stop-backend.ps1"),
        (Join-Path $RepoRoot "scripts\stop-backend.ps1")
    )
