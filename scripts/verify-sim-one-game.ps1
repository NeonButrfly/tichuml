...
Purpose:
  Runs a one-game Windows simulator/backend telemetry smoke check and captures diagnostics.
...
Options:
  ...
  -PostgresUser <name>
  -PostgresDb <name>
  -ClearDatabase          Destructive: truncates telemetry tables before the smoke test.
...
Destructive warning:
  Database clearing only runs when -ClearDatabase is explicitly passed.
"@ | Write-Host
  exit 0
}
