...
if (-not $env:BACKEND_REPO_ROOT -or [string]::IsNullOrWhiteSpace($env:BACKEND_REPO_ROOT)) {
  $candidate = Split-Path -Parent $PSScriptRoot
  if (Test-Path -LiteralPath (Join-Path $candidate "package.json")) {
    $env:BACKEND_REPO_ROOT = $candidate
  } else {
    $env:BACKEND_REPO_ROOT = (Get-Location).Path
  }
}
...
function Test-PostgresContainerIdentity {
  ...
  if ($hasOldUser -or $hasOldDb) {
    throw "Existing $container uses old Postgres identity (POSTGRES_USER=postgres or POSTGRES_DB=tichuml). Run scripts\reset-db.ps1 to recreate it."
  }
}
