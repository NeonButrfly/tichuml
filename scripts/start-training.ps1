...
function Show-HelpText {
  @"
Usage:
  scripts\start-training.ps1 [options]
...
function Ensure-WorkspaceBuilds {
  ...
  Write-Host "Workspace package builds are missing; building required packages before training launch."
...
$stopTrainingScript = Assert-RepoPath -RepoRoot $repoRoot -RelativePath "scripts\\stop-training.ps1" -Description "Training stop script"
$selfScriptPath = Assert-RepoPath -RepoRoot $repoRoot -RelativePath "scripts\\start-training.ps1" -Description "Training start script"
...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($SessionName) -and -not [string]::IsNullOrWhiteSpace($RunName)) {
    $SessionName = $RunName
  }
  $strictTelemetryValue = Convert-ToBooleanValue -Value $StrictTelemetry -Fallback $false
  $trainingRunsRoot = if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    Join-Path $repoRoot "training-runs"
  } else {
    $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($OutputDir)
  }
  Ensure-WorkspaceBuilds -RepoRoot $repoRoot
  ...
  $prepareArgs = @(
    ...
    "--strict-telemetry", "$strictTelemetryValue",
    "--telemetry-mode", "full",
  ...
  if ($existing) {
    if ($ReplaceSession) {
      ...
      throw "Session already exists: $sessionNameResolved`nStop: scripts\stop-training.ps1 -SessionName $sessionNameResolved"
    }
  }
