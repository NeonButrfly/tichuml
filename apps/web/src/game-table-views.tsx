...
export function CardFace({
  ...
  selected?: boolean;
  className?: string;
  dataPassCardOrientation?: "vertical" | "horizontal";
  draggable?: boolean;
  onClick?: () => void;
  ...
  if (interactive) {
    ...
    return (
      <button
        ...
        className={classes}
        draggable={draggable}
        data-pass-card-orientation={dataPassCardOrientation}
        {...buttonProps}
      >
        {cardContent(card)}
      </button>
  ...
  return (
    <div className={classes} data-pass-card-orientation={dataPassCardOrientation}>
      {cardContent(card)}
    </div>
  );
}
...
function PassRouteToken({
  ...
  const anchor = getPassingAnchorById(anchorId);
  const cardOrientation = getPassingAnchorCardOrientation(anchorId);
    ...
    const pickupCardClassName =
      route.displayMode === "pickup"
        ? route.sourcePosition === "top" || route.sourcePosition === "bottom"
          ? route.target === "left"
            ? "normal-card--route-pickup-left"
            : route.target === "right"
              ? "normal-card--route-pickup-right"
              : "normal-card--route-pickup-upright"
          : anchor.lane === "north"
            ? "normal-card--route-pickup-up"
            : anchor.lane === "south"
              ? "normal-card--route-pickup-down"
    ...
    return (
      <CardFace
        ...
        className={[
          "normal-card",
          "normal-card--route",
          route.displayMode === "pickup" ? "normal-card--route-pickup" : "",
          pickupCardClassName,
          `normal-card--route-${cardOrientation}`
        ]
          .filter(Boolean)
          .join(" ")}
        dataPassCardOrientation={cardOrientation}
        {...interactiveProps}
      />
    );
  }
  ...
  return (
    <div
      ...
      style={
        route.occupied
          ? {
              backgroundImage: `url(${CARD_BACK_BLUE_SRC})`
            }
          : undefined
      }
    />
    ...
    >
      {(["top", "right", "bottom", "left"] as const).map((sourcePosition) => {
        ...
        return NORMAL_PASS_STAGE_MAP[sourcePosition].map((laneSpec) => {
          ...
          const anchorId = resolvePassingAnchorId(
            sourcePosition,
            laneSpec.targetPosition
          );
          if (!anchorId) {
              ...
              : anchor.lane === "south"
                ...
                : anchor.lane === "left"
                  ...
                  : anchor.lane === "right"
                    ? "-90deg"
                    : "0deg";
          const clipPath = anchor.polygon_px
            .map(
              (point) =>
                `${((point.x - anchor.bbox_px.x) / Math.max(anchor.bbox_px.width, 1)) * 100}% ${((point.y - anchor.bbox_px.y) / Math.max(anchor.bbox_px.height, 1)) * 100}%`
            )
            .join(", ");
          const laneStyle: CSSProperties = {
            left: `${anchor.bbox_px.x}px`,
            top: `${anchor.bbox_px.y}px`,
            width: `${anchor.bbox_px.width}px`,
            height: `${anchor.bbox_px.height}px`,
            "--normal-pass-visible-rotation": visibleRotation
          };
...
export function NormalGameTableView(props: GameTableViewProps) {
  ...
  const passDebugOverlaySrc = showPassDebugOverlay
    ? toAssetHref(DEBUG_PASSING_OVERLAY_SOURCE_PATH)
    : null;
