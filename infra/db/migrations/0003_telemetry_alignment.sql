ALTER TABLE decisions
  ...
  ADD COLUMN IF NOT EXISTS chosen_action_is_legal BOOLEAN NOT NULL DEFAULT TRUE,
  ...
  ADD COLUMN IF NOT EXISTS explanation_quality_level TEXT NOT NULL DEFAULT 'none',
...
SET
  ...
  chosen_action_is_legal = COALESCE(chosen_action_is_legal, TRUE),
  ...
  explanation_quality_level = CASE
    WHEN COALESCE(state_features, metadata->'explanation'->'stateFeatures', metadata->'policy_explanation'->'stateFeatures') IS NOT NULL THEN 'featured'
    WHEN COALESCE(candidate_scores, metadata->'explanation'->'candidateScores', metadata->'policy_explanation'->'candidateScores') IS NOT NULL THEN 'scored'
    WHEN COALESCE(explanation, metadata->'explanation', metadata->'policy_explanation') IS NOT NULL THEN 'basic'
    ELSE 'none'
  END,
...
WHERE requested_provider IS NULL
  ...
  OR explanation_quality_level = 'none';
...
CREATE INDEX IF NOT EXISTS decisions_requested_provider_idx ON decisions(requested_provider);
...
CREATE INDEX IF NOT EXISTS decisions_legal_actions_hash_idx ON decisions(legal_actions_hash);
...
CREATE INDEX IF NOT EXISTS events_requested_provider_idx ON events(requested_provider);
CREATE INDEX IF NOT EXISTS events_provider_used_idx ON events(provider_used);
...
CREATE OR REPLACE VIEW telemetry_duplicate_state_counts AS
SELECT
  state_hash,
  COUNT(*)::INTEGER AS decision_count
FROM decisions
...
GROUP BY state_hash
HAVING COUNT(*) > 1
ORDER BY decision_count DESC, state_hash ASC;

CREATE OR REPLACE VIEW telemetry_duplicate_legal_action_counts AS
SELECT
  legal_actions_hash,
  COUNT(*)::INTEGER AS decision_count
FROM decisions
...
HAVING COUNT(*) > 1
ORDER BY decision_count DESC, legal_actions_hash ASC;
